...
const totalPrice = dataList.reduce(
  (prevSum, price) => prevSum + price.price,
  0
);
console.log(
  `1) Загальна вартість (нові ціни - price) дорівнює: ${totalPrice} грн.`
);

// 2)Знайти кількість товарів, у яких ціна зменшилась (price < old_price).
const productsQntWithDecreasedPrice = dataList.reduce(
  (prevNum, price) => (price.price < price.old_price ? prevNum + 1 : prevNum),
  0
);
console.log(
  `2) Кількість товарів, у яких ціна зменшилась (price < old_price) - ${productsQntWithDecreasedPrice}шт.`
);

// 3) Товари, які доступні (sell_status:"available")
// var1
// const availableProductsList = dataList.filter(
//   (product) => product.sell_status === "available"
// );
// console.log( `3) Товари, які доступні (sell_status:"available") -${JSON.stringify(availableProductsList)}`)
// var2
...
console.log(
  `3) Товари, які доступні (sell_status:"available") - ${availableProducts}`
);
...
// 4) сформувати новий список об”єктів тільки доступних для продажу товарів, які міститимуть тільки ідентифікатор товару (id), нову ціну (price), стару ціну (old_price), та ціну у доларах (usd_price)
  function getList({ id, price, old_price, usd_price }) {
    return {
      id,
      price,
      old_price,
  ...
  let newAvailableProductsList = [];
  for (const product of dataList) {
    if (product.sell_status === "available") 
    newAvailableProductsList.push(getList(product));
  }
console.log(
  `4) новий список об”єктів - ${JSON.stringify(newAvailableProductsList)}`
);